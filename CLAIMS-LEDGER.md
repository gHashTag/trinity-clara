...
|----|-------|-----------------|--------|----------------------|--------------------|
| F-1 | Total machine-checked theorems across the Trinity program | **2,027 `Qed.` across 100 `.v` files, 0 real `Admitted.`** in `trinity-s3ai` (machine-counted by `scripts/count_admitted_honest.py`, which strips comments before counting; reproducible, 2026-05-29). Sub-totals: `proofs/trinity/` 1,130 + `proofs/trinity/coq_models/` 86 + `proofs/clifford_cl8/` 103 + `proofs/catalog/` 1. The honest counter reports 0 real `Admitted.` in `proofs/trinity/`; `proofs/clifford_cl8/` retains 4 load-bearing `Axiom`s with citations. | `PROVEN where Qed.` | [`trinity-s3ai/scripts/count_admitted_honest.py`](https://github.com/gHashTag/trinity-s3ai/blob/main/scripts/count_admitted_honest.py) (authoritative reproducible counter) | **Quote the counter output, not the README headline.** The `trinity-s3ai` README is internally inconsistent (it carries both "1,325" and "1,762" `Qed.`); both are stale snapshots superseded by the 2,027 machine count. Never conflate the program-wide figure with the CLARA math-core (F-2). Always pair any count with its `Admitted`/`Axiom` total. See [`REPRODUCIBILITY.md`](REPRODUCIBILITY.md). |
...
| ID | Claim | Canonical value | Status | Authoritative source | Notes / correction |
|----|-------|-----------------|--------|----------------------|--------------------|
| R-1 | Adversarial variants blocked (Red Team) | **100% (50/50)**: 25 normal_passed + 25 adversarial_blocked; false_positive_rate 0.0; false_negative_rate 0.0; avg recovery 0.048 ms, max 0.118 ms | `SYNTHETIC, v1.0 testset` (5×5 adversarial + 25 normal balanced cases) | Machine-readable canonical results: [`test_vectors/ta2/redteam_tests.json`](test_vectors/ta2/redteam_tests.json) (metadata: `test_framework: 'CLARA Red Team', version: '1.0', date: '2026-04-15'`); harness: [`examples/05_redteam_test.py`](examples/05_redteam_test.py) and [`evidence/CLARA-RED-TEAM.md`](evidence/CLARA-RED-TEAM.md) §Results. | **Updated 2026-05-30** to match the canonical JSON. The earlier ledger row recorded "96% (48/50), 7.2 ms avg" from an older 100-scenario draft set whose JSON is not present in the repository at the pinned commit; that value is retired. Quote rule: never quote "100%" bare; always pair with the qualifier *"on the v1.0 synthetic Red Team testset (50 balanced cases, 5 attack categories), see `test_vectors/ta2/redteam_tests.json`"*. The score is `SYNTHETIC` — it does **not** generalise to unseen adversaries; ≥95% remains the target for the broader fielded threat model (R-2). |
| R-2 | Red Team target | **≥ 95% on the broader fielded threat model** | `PROJECTED` | Technical Proposal §4.6, [`evidence/CLARA-RED-TEAM.md`](evidence/CLARA-RED-TEAM.md) §Targets (`robustness: 0.95`) | Distinct from R-1: R-1 is the observed v1.0 testset score (`SYNTHETIC`); R-2 is the target for the broader threat model Phase 2 will expand to. Keep both visible together so a reviewer cannot conflate them. |
| R-3 | Attack categories covered | **5** (fuel deception, action exhaustion, timeline manipulation, resource poisoning, trace manipulation); 5 cases per category in v1.0 testset | `SYNTHETIC` | [`evidence/CLARA-RED-TEAM.md`](evidence/CLARA-RED-TEAM.md), [`test_vectors/ta2/redteam_tests.json`](test_vectors/ta2/redteam_tests.json) | Category count matches the JSON. The harness `examples/05_redteam_test.py` lists 3 *additional* (unpublished) attack categories (compression, multi-tier timeline deception, confidence poisoning); these are present in the harness but not in the canonical v1.0 testset. Do not claim "8 categories" anywhere until they ship in a v1.1+ testset JSON. |
...
| ID | Claim | Canonical value | Status | Authoritative source | Notes / correction |
|----|-------|-----------------|--------|----------------------|--------------------|
| H-1 | Energy efficiency vs GPU | **49×** on legacy FPGA (XC7A100T @ 92 MHz vs Jetson Orin baseline) | `MEASURED — evidence pending` (prototype) | §8.5 methodology | Keep the *methodology* sentence attached every time the number appears; never quote "49×" bare. The UART throughput log and Vivado power-report screenshot have not yet been committed to `t27/bench/results_v02_real.json` (which still records `tokens_per_sec_real: null` at pinned commit `e7a07f1`). Until that evidence lands, the tag stays `MEASURED — evidence pending`; see `DISCREPANCIES.md` D-12 and `T27-PIN.md` §3. |
| H-2 | Throughput | 63 tok/s @ 1.2 W (19 mJ/token) | `MEASURED — evidence pending` (prototype) | §8.5 | OK with board + clock stated, but `t27/bench/results_v02_real.json` at the pinned commit still has `tokens_per_sec_real: null` and `bitstream_sha256: TBD`. Same status discipline as H-1 applies until evidence files (UART log, Vivado power report, bitstream SHA-256) are committed and `T27-PIN.md` is re-pinned. |
...
| H-6 | Ε Euler chip identity on TTSKY26b | Project **#558**, top module `tt_um_ghtag_trinity_gf16`, 8×2 tiles | `Verified` (external registry) | [registry](https://tinytapeout.com/chips/ttsky26b/), [tt-trinity-euler](https://github.com/gHashTag/tt-trinity-euler) | Confirmed against live registry 2026-05-29. Retires fabricated "#4915." |
...
| Ε Euler | `tt_um_ghtag_trinity_gf16` | **#558** | ~~#4915~~ | [tt-trinity-euler](https://github.com/gHashTag/tt-trinity-euler) |
